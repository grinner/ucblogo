...
extern "C" int start (int, const char **);
...
wxTerminal::InvertArea(wxDC &dc, int t_x, int t_y, int w, int h, bool scrolled_coord) {
  ...
  // causes crash in wx 3.1
  dc.SetLogicalFunction(wxINVERT);
  dc.Blit( t_x, t_y, w, h, &dc, t_x, t_y);
}
...
void wxTerminal::DebugOutputBuffer() {
  ...
  wxterm_charpos pos_1 = line_of(lpos);
    ...
    fprintf(stderr, "WXTERMINAL STATS: \n  width: %d, height: %d, \n cw: %d, ch: %d \n x_max: %d, y_max: %d \n cursor_x: %d, cursor_y: %d \n last_logo_x : %d, last_logo_y: %d \ncurr_charpos buf: %p offset: %d  \ncurr_line buf: %p offset: %d\n",
            m_width, m_height, m_charWidth, m_charHeight, 
            x_max, y_max, cursor_x, cursor_y, 
            last_logo_x, last_logo_y, curr_char_pos.buf, curr_char_pos.offset, 
            curr_line_pos.buf, curr_line_pos.offset);
  ...
  for(int i = 0; i <= y_max; i++) {
    fprintf(stderr, "LINE %d: buf: %p, offset: %d, len: %d\n", i, line_of(lpos).buf, line_of(lpos).offset, line_of(lpos).line_length);
    inc_linepos(lpos);
  }
    fprintf(stderr, "\n#############\n\n");
}
